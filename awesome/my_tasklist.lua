...
local function tasklist_label(c, args, tb)
	...
	local fg_urgent =
		gcolor.ensure_pango_color(args.fg_urgent or theme.tasklist_fg_urgent or theme.fg_urgent, fg_normal)
	...
	local tasklist_disable_icon = args.disable_icon
		or args.tasklist_disable_icon
	...
	if not disable_task_name then
		if c.minimized then
			name = name
				.. (gstring.xml_escape(c.icon_name) or gstring.xml_escape(c.name) or gstring.xml_escape("<untitled>"))
		else
	...
	if
		not focused
		and capi.client.focus
		and capi.client.focus.skip_taskbar
...
function tasklist.new(args, filter, buttons, style, update_function, base_widget)
	...
	if
		argstype == "number"
		or argstype == "screen"
		or (argstype == "table" and args.index and args == capi.screen[args.index])
	then
